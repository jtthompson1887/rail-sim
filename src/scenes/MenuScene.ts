...
export default class MenuScene extends Phaser.Scene {
  ...
  private previewSolver?: TrackFlowSolver;
  ...
  create(): void {
    ...
    for (let i = 0; i < circleSegments; i++) {
      ...
      trackPoints.push(new Phaser.Math.Vector2(circleCenter.x + Math.cos(angle) * trackRadius, circleCenter.y + Math.sin(angle) * trackRadius));
    }
    ...
    trainBody.setAngle(firstTrack.getTrackAngle(trainBody));
    menuTrain.enginePower = 40;
    this.trains.push(menuTrain);
    this.previewSolver = new TrackFlowSolver(this.railTracks, menuTrain);

    const panelWidth = width * 0.42;
    const panelHeight = height * 0.72;
    const panelX = width * 0.72;
    const panelY = height * 0.5;
    this.add.rectangle(panelX, panelY, panelWidth, panelHeight, 0x031626, 0.82).setStrokeStyle(4, 0xffffff, 0.2).setScrollFactor(0).setDepth(100);
    this.add.rectangle(panelX, panelY - panelHeight * 0.36, panelWidth * 0.6, 6, 0x4ad5ff, 0.45).setScrollFactor(0).setDepth(101);
  ...
  update(time: number, delta: number): void {
    ...
    this.previewSolver?.applyTrackFlowForces();
  }
